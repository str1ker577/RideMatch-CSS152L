...
def clean_brand_data(df):
    """Clean up duplicate and problematic brand entries"""
    
    if df.empty:
        return df
    
    app.logger.info("🧹 Cleaning brand data...")
    
    # Count original brands
    original_brands = df['Brand'].value_counts()
    app.logger.info(f"Original brand counts: {original_brands.to_dict()}")
    
    # Fix Mercedes-Benz duplicate issue
    df.loc[df['Brand'].str.contains('Mercedes-Benz', case=False, na=False), 'Brand'] = 'Mercedes'
    app.logger.info("✅ Mercedes-Benz entries converted to Mercedes")
    
    # Remove duplicate Ford entries (keep only distinct Ford entries)
    ford_duplicates = df[df['Brand'].str.contains('Ford', case=False, na=False)]
    if len(ford_duplicates) > len(ford_duplicates['Model'].unique()):
        app.logger.info(f"Found {len(ford_duplicates)} Ford entries with {len(ford_duplicates['Model'].unique())} unique models")
        # Remove exact duplicates, keep first occurrence
        df = df.drop_duplicates(subset=['Brand', 'Model', 'Variant'], keep='first')
        app.logger.info("✅ Ford duplicates removed")
    ...
    for old_name, new_name in brand_standardization.items():
        df.loc[df['Brand'].str.lower() == old_name.lower(), 'Brand'] = new_name
    
    # Count final brands
    final_brands = df['Brand'].value_counts()
    app.logger.info(f"Final brand counts: {final_brands.to_dict()}")
    
    return df

# Update your load_csv_data_enhanced function
def load_csv_data_enhanced():
    """Load CSV data with detailed tracking and row preservation"""
    global df
    ...
    try:
        ...
        # After successful CSV loading, add this cleanup step:
        app.logger.info("🧹 STEP 6: Cleaning brand data")
        df_cleaned = clean_brand_data(df_processed)
        
        # Set the global DataFrame
        df = df_cleaned
        app.logger.info("✅ CSV data loading, processing, and cleaning completed successfully")
        return True
...
# Add a new route to get cleaned brands
@app.route('/get_brands_clean', methods=['GET'])
def get_brands_clean():
    """Get cleaned brands list without duplicates"""
    if df.empty:
        app.logger.warning("No car data available for brands")
        return jsonify([])
    ...
    try:
        ...
        brands = df["Brand"].dropna().unique().tolist()
        brands = [brand for brand in brands if brand.strip()]  # Remove empty strings
        brands.sort()
        
        # Log for debugging
        app.logger.info(f"Clean brands returned: {brands}")
        
        return jsonify(brands)
    except Exception as e:
        app.logger.error(f"Error getting clean brands: {e}")
        return jsonify([])

# Add manual cleanup route for testing
@app.route('/cleanup_brands', methods=['POST'])
def manual_cleanup_brands():
    """Manually trigger brand cleanup"""
    global df
    ...
    try:
        ...
        original_brands = df['Brand'].value_counts().to_dict()
        
        # Apply cleanup
        df = clean_brand_data(df)
        
        final_count = len(df)
        final_brands = df['Brand'].value_counts().to_dict()
        ...
        return jsonify({
            ...
            "message": f"Cleaned {original_count - final_count} duplicate entries"
        })
