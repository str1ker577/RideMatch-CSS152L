...
async function initializeProfilePage() {
    ...
    try {
        ...
        showProfileMessage('Error loading profile. Please refresh the page.', 'error');
    }
}
...
async function loadUserProfile(user) {
    try {
        ...
        currentProfileData = { ...originalProfileData };
...
function updateProfileUI() {
    ...
    if (profilePic) {
        if (currentProfileData.profilePicture) {
            profilePic.src = currentProfileData.profilePicture;
            profilePic.style.display = 'block';
        } else {
            // Set default avatar
            profilePic.src = 'data:image/svg+xml;base64,';
        }
    }
    ...
    if (usernameInput) {
        usernameInput.value = currentProfileData.username;
    }
    ...
    if (memberSinceSpan) {
        memberSinceSpan.textContent = currentProfileData.memberSince;
    }

    // Update welcome message with username
    updateWelcomeMessage(currentProfileData.username);
}
...
async function handleProfilePictureUpload(event) {
    ...
    try {
        showProfileMessage('Uploading image...', 'info');
        ...
        reader.onload = (e) => {
            ...
            if (profilePic) {
                ...
                currentProfileData.profilePicture = e.target.result;
                detectChanges();
            }
        };
        ...
        showProfileMessage('Profile picture updated! Remember to save changes.', 'success');

    } catch (error) {
        console.error('Error uploading profile picture:', error);
        showProfileMessage('Error uploading image. Please try again.', 'error');
    }
}

// Enable editing for a field
function enableEdit(fieldName) {
    const input = document.getElementById(`profile-${fieldName}`);
    const actions = document.getElementById(`${fieldName}-actions`);
    const editBtn = input.parentElement.querySelector('.edit-btn');

    if (input && actions && editBtn) {
...
async function saveField(fieldName) {
    ...
    if (!input || !actions || !editBtn) return;

    const newValue = input.value.trim();
    
    if (!newValue) {
        showProfileMessage(`${fieldName.charAt(0).toUpperCase() + fieldName.slice(1)} cannot be empty.`, 'error');
        return;
    }

    try {
        // Update current data
        currentProfileData[fieldName] = newValue;
        ...
        detectChanges();
        showProfileMessage(`${fieldName.charAt(0).toUpperCase() + fieldName.slice(1)} updated! Remember to save all changes.`, 'success');

    } catch (error) {
        console.error(`Error saving ${fieldName}:`, error);
        showProfileMessage(`Error updating ${fieldName}. Please try again.`, 'error');
    }
}
...
function cancelEdit(fieldName) {
    ...
    if (input && actions && editBtn) {
        input.value = originalProfileData[fieldName] || currentProfileData[fieldName];
        input.readOnly = true;
        input.style.background = '#f8f9fa';
        actions.style.display = 'none';
        editBtn.style.display = 'block';
...
function togglePasswordVisibility() {
    ...
    if (passwordInput.type === 'password') {
        // Show password
        if (auth && auth.currentUser) {
            // In a real app, you wouldn't store/show the actual password
            passwordInput.value = '••••••••••••'; // Keep masked for security
...
function detectChanges() {
    const currentUsername = document.getElementById('profile-username').value.trim();
    
    hasUnsavedChanges = (
        currentUsername !== originalProfileData.username ||
        currentProfileData.profilePicture !== originalProfileData.profilePicture
    );

    // Update save button state
    const saveAllBtn = document.querySelector('.save-all-btn');
    if (saveAllBtn) {
        saveAllBtn.style.opacity = hasUnsavedChanges ? '1' : '0.6';
...
async function saveAllChanges() {
    ...
    if (!auth || !auth.currentUser) {
        showProfileMessage('Please log in to save changes.', 'error');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            if (currentProfileData.profilePicture) {
                ...
                if (headerProfilePic) {
                    headerProfilePic.src = currentProfileData.profilePicture;
            ...
            showProfileMessage('Changes saved successfully!', 'success');
            detectChanges(); // Update button states
        } else {
        ...
        showProfileMessage('Error saving changes. Please try again.', 'error');
    }
}
...
function cancelAllChanges() {
    if (!hasUnsavedChanges) {
        showProfileMessage('No changes to cancel.', 'info');
        return;
    }
    ...
    showProfileMessage('Changes cancelled.', 'info');
}
...
async function loadUserStats() {
    try {
        ...
        if (response.ok) {
            ...
            if (favoritesCount) favoritesCount.textContent = stats.favorites || 0;
            if (postsCount) postsCount.textContent = stats.posts || 0;
            if (testimonialsCount) testimonialsCount.textContent = stats.testimonials || 0;
        }
    } catch (error) {
...
// Updated signup handler to include username
function handleSignup(event) {
    event.preventDefault();
    ...
    if (!username) {
        document.querySelector('.error-message12').textContent = 'Username is required.';
        return false;
    }
    ...
    auth.createUserWithEmailAndPassword(email, password)
        .then((userCredential) => {
            ...
            saveUsernameToBackend(user.uid, username, email);
...
// Save username to backend
async function saveUsernameToBackend(userId, username, email) {
    try {
        ...
        if (!response.ok) {
            console.error('Failed to save username to backend');
        }
    } catch (error) {
...
// Updated welcome message function
...
// Updated auth state change handler in initializeFirebase
const originalInitializeFirebase = window.initializeFirebase;
window.initializeFirebase = async function() {
    if (originalInitializeFirebase) {
        await originalInitializeFirebase();
    } else {
            ...
            if (user) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        updateWelcomeMessage(user.email.split('@')[0]);
                    }
                } catch (error) {
                ...
                // Rest of original auth state handling
                userName = user.email;
                const loginBtn = document.getElementById('login-button');
                const profileContainer = document.getElementById('profile-container');
                if (loginBtn) loginBtn.style.display = 'none';
